...
import com.activiti.bpmn.manager.image.CustomProcessDiagramGenerator;
...
public class ProcessImageManager {
    ...
    public InputStream getImgByDeploymentId(String deploymentId)throws Exception{
        if(StringUtils.isEmpty(deploymentId)){
            log.error("[错误]-传入的参数deploymentId为空！");
            throw new Exception("[异常]-传入的参数deploymentId为空！");
        }
        ProcessDefinition pd = repositoryService.createProcessDefinitionQuery().deploymentId(deploymentId).latestVersion().singleResult();
        BpmnModel bm = repositoryService.getBpmnModel(pd.getId());
        ProcessDiagramGenerator processDiagramGenerator = new DefaultProcessDiagramGenerator();
        return processDiagramGenerator.generateDiagram(bm,new ArrayList<>(),new ArrayList<>(),"宋体", "微软雅黑", "黑体"); // 生成图片，获取图片的输入流
    }
