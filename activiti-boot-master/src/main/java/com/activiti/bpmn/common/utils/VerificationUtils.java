package com.activiti.bpmn.common.utils;
...
public class VerificationUtils {
    ...
    public static ProcessDefinition checkProcessDefinitionKey(String processDefinitionKey) {
        // 不能在list后直接get，因为如果没有流程定义的话，会报错
        List<ProcessDefinition> processDefinition = ActivitiUtil.instance().getRepositoryService().createProcessDefinitionQuery()
                ...
                .list();
        // 判断错误并抛出
        FunctionUtils.isWrong(CollectionUtils.isEmpty(processDefinition)).throwCustomException("流程定义" + processDefinitionKey + "未找到");
        FunctionUtils.isWrong(processDefinition.size() > 1).throwCustomException("根据给定的流程名称" + processDefinitionKey + ", 查找到多个流程定义");
        return processDefinition.get(0);
    }
